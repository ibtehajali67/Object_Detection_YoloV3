...
car, person, bicycle, motorbike, bus, truck =1,1,1,1,1,1
temp = 1
import os
import easyocr
car_text = open('./IMAGES/results/car/car.txt','a')
...
def draw_bbox(image, bboxes, output_directory_path, CLASSES=YOLO_COCO_CLASSES, show_label=True, show_confidence=True,
    ...
    global bus
    global truck
    file_name = output_directory_path.rsplit('.')[-1].rsplit('/')[-1]
    if not os.path.exists('./IMAGES/' + file_name):
        os.mkdir('./IMAGES/' + file_name)
    output_directory_path = './IMAGES/' + file_name
    ...
    for i, bbox in enumerate(bboxes):
        ...
        if NUM_CLASS[class_ind] in ['car', 'person', 'bicycle', 'motorbike', 'bus', 'truck']:
            crop_img = image[y1:y2, x1:x2]

            if NUM_CLASS[class_ind] == 'car':

                cv2.imwrite(output_directory_path + '/car/' + str(car) + '.jpg', crop_img)
                car+=1

            if NUM_CLASS[class_ind] == 'person':
                cv2.imwrite(output_directory_path + '/person/' + str(person) + '.jpg', crop_img)
                person+=1

            if NUM_CLASS[class_ind] == 'bicycle':
                cv2.imwrite(output_directory_path + '/bicycle/' + str(bicycle) + '.jpg', crop_img)
                bicycle+=1

            if NUM_CLASS[class_ind] == 'motorbike':
                cv2.imwrite(output_directory_path + '/motorbike/' + str(motorbike) + '.jpg', crop_img)
                motorbike+=1

            if NUM_CLASS[class_ind] == 'bus':
                cv2.imwrite(output_directory_path + '/bus/' + str(bus) + '.jpg', crop_img)
                bus+=1

            if NUM_CLASS[class_ind] == 'truck':
                cv2.imwrite(output_directory_path + '/truck/' + str(truck) + '.jpg', crop_img)
                truck+=1
            # put object rectangle
            # image cropping

           # cv2.imwrite(output_directory_path + '/' + str(temp) + '.jpg', crop_img)
            #   croped_image = cv2.imread(crop_img)
            try:
                crop_img = cv2.cvtColor(crop_img, cv2.COLOR_BGR2GRAY)
                result = reader.readtext(crop_img)
                detected_value = result[0][1] + result[1][1]
                if len(detected_value) != 0:
                    car_text.write(detected_value+'\n')
                    ...
                    if show_label:
                        ...
                        (text_width, text_height), baseline = cv2.getTextSize(detected_value,
                        ...
                        cv2.putText(image, detected_value, (x1, y1 - 4), cv2.FONT_HERSHEY_COMPLEX_SMALL,
...
def detect_image(Yolo, image_path, output_directory_path, output_path, input_size=416, show=False,
    ...
    image = draw_bbox(original_image, bboxes, output_directory_path, CLASSES=CLASSES, rectangle_colors=rectangle_colors)
...
def detect_video(Yolo, video_path, output_directory_path, output_path, input_size=416, show=False,
                 CLASSES=YOLO_COCO_CLASSES, score_threshold=0.96, iou_threshold=0.90, rectangle_colors=''):
    times, times_2 = [], []
    vid = cv2.VideoCapture(video_path)
    ...
    while True:
        ...
        image = draw_bbox(original_image, bboxes, output_directory_path, CLASSES=CLASSES,
